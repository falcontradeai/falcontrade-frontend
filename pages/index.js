...
export default function Home() {
  ...
  const cifUSD = (price) => {
    ...
    const ins = price * 0.01 // 1% insurance baseline
  ...
  return (
    <div className="space-y-6">
      <div className="card">
        <div className="badge mb-4">FalconTrade v0.2.1 LIVE — API: {API_BASE}</div>
        ...
        {!loading && !error && (
          <div className="card">
            ...
            <table className="table">
              <thead>
                <tr>
                  <th>Commodity</th><th>Unit</th><th>Price</th><th>Source</th><th>CIF (USD)</th><th>CIF (AZN)</th>
                </tr>
              </thead>
              <tbody>
                {commodities.map(c => (
                  <tr key={c.id}>
                    ...
                    <td>${c.price}</td>
                    <td>{c.source}</td>
                    <td>${cifUSD(c.price)}</td>
                    <td>{cifAZN(c.price)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
