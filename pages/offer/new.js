...
export default function NewOffer() {
  ...
  const submit = async (e) => {
    ...
    const res = await fetch(`${API_BASE}/listings`, {
      ...
      body: JSON.stringify({ type:'OFFER', ...form, details })
    })
    const data = await res.json()
    if (!res.ok) { setMsg(data.detail || 'Failed (login?)'); return }
    setMsg('Created Offer (draft). Admin will publish.')
  }
  ...
  return (
    <div className="card">
      ...
      <form onSubmit={submit} className="space-y-2">
        ...
        <textarea className="textarea" rows={6} placeholder="Details JSON (e.g., {"grade":"A","spec":"..."})" onChange={e=>{
